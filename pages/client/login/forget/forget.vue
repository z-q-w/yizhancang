<template>
	<view class="container">
		...
		<form @submit="" class="inpt">
			<input type="text" value="" v-model="account" placeholder="请输入手机号" />
			<!-- <view class="code">
			...
			<view class="psd">
				<input type="text" value="" v-model="password" password=true placeholder="请输入密码" />
			</view>
			<view class="psd">
				<input type="text" value="" v-model="surePassword" password=true placeholder="请再次输入密码" />
			</view>
			<button form-type="submit" class="inreg" @click="forgetPwd">确定</button>
		</form>
		<view class="operating">
			<view class="left">
				已有账号?去<navigator url="../login" class="" class="reg" style="color: #ffc300;">登录</navigator>
			</view>
			<navigator url="../reg/reg" class="forget">注册</navigator>
		</view>
	</view>
</template>
			...
			btnSend(){//发送验证码
				if (!this.utils.isMobile(this.account)) {
					...
					this.api.messagesend({
						mobile:this.account,
						type:'4' //1 1 用户注册 2 用户登录 3 修改密码  4 忘记密码
					}).then(res=>{
