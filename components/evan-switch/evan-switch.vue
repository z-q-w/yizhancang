<template>
	<view @click="toggle" class="evan-switch" :class="{'evan-switch--disabled':disabled}" :style="{width:2*size+'px',height:switchHeight,borderRadius:size+'px',backgroundColor:currentValue===activeValue?activeColor:inactiveColor}">
		...
		<text v-if="hint" class="yes" style="font-size: 25rpx; color: #041b7e;">是</text>
		<text v-if="hint" class="no" style="font-size: 25rpx; color: #d4d4d4;">否</text>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			toggleValue() {
				...
				uni.$emit('change', this.currentValue)
			}
		}
	}
</script>
